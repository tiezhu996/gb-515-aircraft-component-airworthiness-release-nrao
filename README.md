...
- 放行前证据闸门：提交复核与复核批准时按关联部件编码核对部件、检查任务和证书记录，部件必须处于 `hold`、检查必须 `passed`、证书必须 `valid` 且在有效期内；任一条件不满足即拒绝并保留原状态，响应与页面列出具体阻断项。
- 批准时重新读取最新证据，等待复核期间的证据变化会阻断放行；重复或并发批准由乐观锁保证只有一次成功。
