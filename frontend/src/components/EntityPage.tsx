...
export function EntityPage({ config, useStore, certificateRecords = [] }: { config: EntityConfig; useStore: EntityStore; certificateRecords?: DomainRecord[] }) {
	const { items, meta, loading, error, blockers, load, createRecord, transition } = useStore();
  ...
  const createDemo = async () => {
    ...
    {blockers.length > 0 && <div className="alert" role="alert"><strong>放行前证据闸门阻断，请先处理以下事项：</strong><ul className="blocker-list">{blockers.map((blocker) => <li key={blocker}>{blocker}</li>)}</ul></div>}
    <section className="table-shell" aria-busy={loading}><table><thead><tr><th>编码</th><th>名称</th><th>状态</th><th>风险</th><th>责任人</th><th>指标</th><th>更新时间</th><th>操作</th></tr></thead><tbody>
      ...
      {!items.length && !loading && <tr><td colSpan={8} className="empty">暂无记录</td></tr>}
		...
		<ConfirmDialog open={Boolean(pending)} title="确认状态迁移" onCancel={() => setPending(null)} onConfirm={() => { if (pending) void transition(config.path, pending.item, pending.status).catch(() => undefined).finally(() => setPending(null)); }}><p>状态迁移会写入不可覆盖的版本与审计日志。</p><strong>{pending?.item.status} → {pending?.status}</strong></ConfirmDialog>
	</main>;
}
