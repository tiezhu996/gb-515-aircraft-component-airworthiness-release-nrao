...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	evidenceGate := service.NewEvidenceGate(aircraftPartRepository, inspectionTaskRepository, certificateRecordRepository)
	releaseAuthorizationService := service.NewReleaseAuthorizationService(releaseAuthorizationRepository, evidenceGate, securityService)
